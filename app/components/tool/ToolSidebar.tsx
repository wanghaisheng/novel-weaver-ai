...
export const ToolSidebar: React.FC<ToolSidebarProps> = ({
  ...
  // Using CSS variables for sidebar specific theme
  ...
  const navButtonBase = `w-full flex items-center space-x-3 px-3 py-3 rounded-lg transition-all duration-200 ease-in-out group relative focus:outline-none focus:ring-2 focus:ring-offset-1`;
  const navButtonFocusRing = `focus:ring-sidebar-ring focus:ring-offset-sidebar-background`; // Use sidebar ring color
  ...
  const getNavButtonDynamicStyles = (isActive: boolean): React.CSSProperties => {
    if (isActive) {
      return {
        ...
        // boxShadow: '0 0 10px hsl(var(--sidebar-primary) / 0.5)', // Optional shadow
        // borderLeft: '4px solid hsl(var(--sidebar-primary))', // Already used border-l-4
      };
    }
    return {
  ...
  return (
    <div
      ...
      style={sidebarStyles}
    >
      <div className={`p-4 border-b flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`} style={{borderColor: 'hsl(var(--sidebar-border))'}}>
        {!isCollapsed && (
          <div className="flex-grow min-w-0">
            <h1 className="text-xl md:text-2xl font-extrabold cursor-pointer truncate tracking-tight" style={{color: 'hsl(var(--sidebar-primary))'}} onClick={() => { onNavigateHome();}} title={t('toolSidebar.backToHomeTitle', { appTitle: t('appTitle')})}>
              {t('appTitle')}
            </h1>
            <p className="text-xs truncate" style={{color: 'hsl(var(--sidebar-foreground) / 0.7)'}}>{t('toolSidebar.creativeStudio')}</p>
          </div>
        )}
        <button
        ...
        >
          {isCollapsed ? (
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 md:w-6 md:h-6 mx-auto" style={{color: 'hsl(var(--sidebar-foreground) / 0.7)'}}><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
          ) : (
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 md:w-6 md:h-6" style={{color: 'hsl(var(--sidebar-foreground) / 0.7)'}}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>
          )}
        </button>
      </div>
        ...
        {TOOL_PAGE_SECTIONS.map((section) => (
          <React.Fragment key={section.id}>
            <button
              ...
              className={`${navButtonBase} ${navButtonFocusRing} ${getNavButtonHoverStyles(activeToolId === section.id && (activeDisplayId === section.id || (section.id === 'novel-editor' && NOVEL_EDITOR_SUB_SECTIONS.some(sub => sub.id === activeDisplayId))))} ${isCollapsed ? 'justify-center' : ''} ${activeDisplayId === section.id && activeToolId === section.id ? 'border-l-4' : ''}`}
              style={{
                ...getNavButtonDynamicStyles(activeToolId === section.id && (activeDisplayId === section.id || (section.id === 'novel-editor' && NOVEL_EDITOR_SUB_SECTIONS.some(sub => sub.id === activeDisplayId)))),
                borderColor: (activeToolId === section.id && (activeDisplayId === section.id || (section.id === 'novel-editor' && NOVEL_EDITOR_SUB_SECTIONS.some(sub => sub.id === activeDisplayId)))) ? 'hsl(var(--sidebar-primary))' : 'transparent',
              }}
              aria-current={activeToolId === section.id && (activeDisplayId === section.id || (section.id === 'novel-editor' && NOVEL_EDITOR_SUB_SECTIONS.some(sub => sub.id === activeDisplayId))) ? 'page' : undefined}
            ...
            {section.id === 'novel-editor' && activeToolId === 'novel-editor' && !isCollapsed && (
              <div className="pl-4 mt-1 space-y-1 ml-3 py-1" style={{borderLeft: `2px solid hsl(var(--sidebar-border))`}}>
            ...
            <div className="px-3">
              ...
              <input
                  ...
                  style={{
                    ...
                    borderColor: 'hsl(var(--sidebar-border))',
                    // placeholder color would ideally be a lighter version of sidebar-foreground
                  }}
              />
              <div className="flex space-x-2 mt-2">
                  ...
                  <button
                      ...
                      className={`flex-1 px-3 py-1.5 text-xs font-semibold rounded-md shadow-sm focus:outline-none focus:ring-1 ${navButtonFocusRing} ${getNavButtonHoverStyles(false)}`}
                      style={{backgroundColor: 'hsl(var(--sidebar-accent))', color: 'hsl(var(--sidebar-accent-foreground))'}}
                  >
      ...
      <div className={`p-3 mt-auto ${isCollapsed ? 'overflow-x-hidden' : ''}`} style={{borderTop: `1px solid hsl(var(--sidebar-border))`}}>
        ...
        {currentUser && (
            ...
            `}
            style={{backgroundColor: 'hsl(var(--sidebar-accent) / 0.5)', color: 'hsl(var(--sidebar-foreground))'}}
