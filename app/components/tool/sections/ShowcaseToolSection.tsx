      ...
      <div className="px-4 md:px-6">
        ...
        <div className="space-y-8">
          {showcases.map((showcase, index) => (
            <div key={index} className={`p-6 bg-slate-700 rounded-xl shadow-lg border-l-4 ${showcase.borderColor}`}>
              <h3 className="text-xl font-semibold text-slate-100 mb-4">{showcase.title}</h3>
              <div className="mb-4">
                <p className="text-sm font-medium text-slate-300 mb-1.5">{showcase.userInputLabel}</p>
                <div className="bg-slate-900/70 p-3 rounded-md text-slate-300 text-sm italic border border-slate-600">
                  {showcase.userInput}
                </div>
              </div>
              <div>
                <p className="text-sm font-medium text-cyan-300 mb-1.5">{showcase.aiOutputLabel}</p>
                <div className="bg-slate-900/80 p-3 rounded-md text-cyan-200 text-sm whitespace-pre-line border border-slate-600">
                  {showcase.aiOutput}
                </div>
              </div>
            </div>
