...
const SignUpPage: React.FC<SignUpPageProps> = ({ onNavigateToSignIn, onNavigateHome }) => {
  ...
  const handleOAuthSignUp = (provider: OAuthProvider) => {
    ...
    // OAuth sign-up typically uses the same authorization flow as sign-in.
    // The OpenAuth server handles distinguishing new vs. existing users.
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8 cursor-pointer" onClick={() => { console.log('[SignUpPage.tsx] App title clicked, navigating home.'); onNavigateHome(); }}>
          ...
          <p className="text-muted-foreground">{t(currentStep === 'register' ? 'signUpPage.titleRegister' : 'signUpPage.titleVerify')}</p>
        </div>
        ...
        {currentStep === 'register' && (
          ...
          >
            ...
            >
              {isLoading ? <LoadingSpinner /> : t('signUpPage.buttons.register')}
            </button>
          </form>
        )}
        ...
        {currentStep === 'verify' && (
          ...
          >
            <p className="text-sm text-foreground text-center">
              {t('signUpPage.messages.verificationInstructions', { email: email })}
            </p>
            <TextInput
            ...
            <button
              ...
              className="w-full px-6 py-3 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold rounded-lg shadow-lg hover:shadow-primary/30 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-card active:scale-[0.98] disabled:bg-muted disabled:text-muted-foreground disabled:cursor-not-allowed disabled:shadow-none disabled:opacity-70 transition-all duration-200 ease-in-out"
            >
              {isLoading ? <LoadingSpinner /> : t('signUpPage.buttons.verify')}
            </button>
            <button
              type="button"
              onClick={() => { setError(null); setSuccessMessage(null); setCurrentStep('register'); }}
              className="w-full text-sm text-muted-foreground hover:text-primary hover:underline focus:outline-none"
            >
              {t('signUpPage.buttons.backToRegister')}
            </button>
          </form>
        )}
        ...
        <div className="mt-6">
            <div className="relative my-4">
                ...
                <span className="px-2 bg-background text-muted-foreground">{t('signUpPage.orSignUpWith')}</span>
                </div>
            </div>
