...
const NavLink: React.FC<{ href: string; children: React.ReactNode; isButton?: boolean; onClick?: () => void }> = ({ href, children, isButton, onClick }) => (
  <a
    ...
    className={`transition-all duration-200 ease-in-out px-4 py-2 rounded-lg text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 active:scale-[0.98]
                ${isButton 
    ...
    return (
        <>
            {originalPriceStr && priceStr !== originalPriceStr && billingCycle === 'monthly' && (
                <span className="text-base line-through text-slate-500 mr-2">{selectedCurrency.symbol}{originalPriceStr}</span>
            )}
            {billingCycle === 'yearly' && originalPriceStr && plan.id !== 'free' && (
                 <span className="text-base line-through text-slate-500 mr-2">{selectedCurrency.symbol}{originalPriceStr}</span>
            )}
            <span className="text-3xl md:text-4xl font-bold text-slate-100">{selectedCurrency.symbol}{priceStr}</span>
            <span className="text-slate-400 text-sm"> {t('pricing.perMonth')}</span>
            {billingCycle === 'yearly' && plan.id !== 'free' && <span className="text-xs text-slate-500 block">({t('pricing.billedAnnually')})</span>}
        </>
    );
  };
  ...
  const PaymentMethodLogos = () => (
    <div className="flex flex-wrap justify-center items-center gap-4 md:gap-6 mt-4">
      {PAYMENT_METHODS.map(method => (
        <span key={method.name} className="text-slate-400 text-sm border border-slate-700 bg-slate-800 px-3 py-1.5 rounded-md shadow-sm">{method.name}</span>
      ))}
       <span className="text-slate-500 text-xs">{t('pricing.morePaymentMethods')} &gt;&gt;</span>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col items-center">
      ...
      <main className="w-full flex-grow">
        <section className="py-16 md:py-24 bg-gradient-to-b from-slate-900 to-slate-950 relative">
           <div className="absolute inset-0 opacity-5"> <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg"><defs><pattern id="pricingGrid" width="30" height="30" patternUnits="userSpaceOnUse"><path d="M 30 0 L 0 0 0 30" fill="none" stroke="rgba(56, 189, 248, 0.2)" strokeWidth="0.5"/></pattern></defs><rect width="100%" height="100%" fill="url(#pricingGrid)" /></svg></div>
          <div className="container mx-auto px-4 text-center relative z-10">
            <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-100 mb-6 tracking-tight">{t('pricing.pageTitle')}</h1>
            <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-3">
              <div className="relative">
                <select value={selectedCurrency.code} onChange={handleCurrencyChange} className="appearance-none bg-slate-800 border border-slate-700 text-slate-200 text-sm rounded-lg pl-4 pr-10 py-2.5 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 shadow-md" aria-label={t('pricing.selectCurrencyAriaLabel')}>
              ...
              <div className="flex bg-slate-800 p-1 rounded-lg shadow-md border border-slate-700">
                {(Object.keys(BILLING_CYCLE_OPTIONS) as BillingCycle[]).map(cycle => (
                  <button key={cycle} onClick={() => setBillingCycle(cycle)} className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-1 focus:ring-offset-slate-800 ${billingCycle === cycle ? 'bg-sky-600 text-white' : 'text-slate-300 hover:bg-slate-700'}`}>
                    {t(BILLING_CYCLE_OPTIONS[cycle].labelKey)}
                    {cycle === 'yearly' && BILLING_CYCLE_OPTIONS.yearly.saveBadgeKey && (<span className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${billingCycle === cycle ? 'bg-white/20 text-white' : 'bg-pink-500 text-white'}`}>{t(BILLING_CYCLE_OPTIONS.yearly.saveBadgeKey)}</span>)}
                  </button>
                ))}
              </div>
            </div>
            {autoDetectedMessage && <p className="text-xs text-slate-400 mb-10 italic">{autoDetectedMessage}</p>}

            <div className="grid md:grid-cols-3 gap-8 items-end max-w-6xl mx-auto">
              {PRICING_PLANS.map(plan => (
                <div key={plan.id} className={`p-6 md:p-8 rounded-xl shadow-2xl flex flex-col text-left transition-all duration-300 transform hover:scale-[1.02] ${plan.isHighlighted ? 'bg-slate-800 border-2 border-sky-500 relative -translate-y-0 md:-translate-y-4' : 'bg-slate-800/70 border border-slate-700'}`}>
                  {plan.isHighlighted && plan.highlightBadgeKey && billingCycle === 'yearly' && (<div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-pink-500 text-white text-xs font-semibold px-3 py-1 rounded-full shadow-lg">{t(plan.highlightBadgeKey)}</div>)}
                  <h3 className="text-2xl font-semibold text-sky-400 mb-2">{t(plan.nameKey)}</h3>
                  <div className="mb-6 h-16 flex items-center">{formatPrice(plan)}</div>
                  <button onClick={() => handlePlanCtaClick(plan)} className={`w-full py-3 font-semibold rounded-lg shadow-md transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 active:scale-[0.98] ${plan.isHighlighted ? 'bg-pink-500 hover:bg-pink-600 text-white focus:ring-pink-400' : 'bg-slate-700 hover:bg-slate-600 text-slate-100 focus:ring-slate-500'} mb-8`}>
                    {t(plan.ctaTextKey)}
                  </button>
                  <ul className="space-y-2 text-sm text-slate-300 flex-grow">
                    {plan.featureKeys.map((featureKey, idx) => (<li key={idx} className="flex items-start"><svg className="w-5 h-5 text-green-400 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg><span>{t(featureKey)} <small className="text-slate-500">(i)</small></span></li>))}
                  </ul>
                </div>
              ))}
            </div>
             <p className="text-xs text-slate-500 mt-8">{t('pricing.polloApiNote')} <a href="#" className="text-sky-400 hover:underline">{t('pricing.polloApiLinkText')}</a>.</p>
             <div className="mt-8 text-sm text-slate-400"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 inline mr-1" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 1a4.5 4.5 0 00-4.5 4.5V9H5a2 2 0 00-2 2v6a2 2 0 002 2h10a2 2 0 002-2v-6a2 2 0 00-2-2h-.5V5.5A4.5 4.5 0 0010 1zm3 8V5.5a3 3 0 10-6 0V9h6z" clipRule="evenodd" /></svg>{t('pricing.paymentSafety')}</div>
            <PaymentMethodLogos />
          </div>
        </section>
        ...
        <section className="py-16 md:py-20 bg-slate-900 border-t border-slate-800">
          <div className="container mx-auto px-4 text-center">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-100 mb-6 tracking-tight">{t('pricing.joinUsersSection.title', { appTitle: t('appTitle')})}</h2>
            <button onClick={handleStartCreatingButton} className="px-8 py-3 bg-gradient-to-r from-sky-500 to-cyan-400 hover:from-sky-600 hover:to-cyan-500 text-white font-semibold rounded-lg shadow-lg hover:shadow-cyan-500/30 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 focus:ring-offset-slate-900 active:scale-[0.98]">
        ...
        <section className="py-12 bg-slate-950">
            <div className="container mx-auto px-4"><div className="flex flex-wrap justify-center items-center gap-x-8 md:gap-x-12 gap-y-4">{TRUSTED_COMPANIES_LOGOS.map(company => (<span key={company} className="text-slate-500 font-medium text-lg filter grayscale opacity-60 hover:grayscale-0 hover:opacity-100 transition-all">{company}</span>))}</div></div>
        </section>

        <section id="pricing-faqs" className="py-16 md:py-20 bg-slate-900 border-t border-slate-800 scroll-mt-20">
          <div className="container mx-auto px-4 max-w-3xl">
            <h2 className="text-3xl font-bold text-center text-sky-300 mb-12 tracking-tight">{t('faqs.pricing.sectionTitle')}</h2>
            <div className="space-y-3">
              {PRICING_FAQS.map((faq, index) => (
                <div key={index} className="bg-slate-800 rounded-lg shadow-md border border-slate-700">
                  <button onClick={() => toggleFaq(index)} className="flex justify-between items-center w-full p-5 text-left hover:bg-slate-700/50 rounded-t-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-1 focus:ring-offset-slate-800" aria-expanded={openFaqIndex === index} aria-controls={`faq-answer-${index}`}>
                    <span className="text-md font-medium text-slate-100">{t(faq.questionKey)}</span><span className={`transform transition-transform duration-300 text-slate-400 ${openFaqIndex === index ? 'rotate-180' : 'rotate-0'}`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg></span>
                  </button>
                  {openFaqIndex === index && (<div id={`faq-answer-${index}`} className="p-5 border-t border-slate-700"><p className="text-slate-300 text-sm leading-relaxed">{t(faq.answerKey)}</p></div>)}
                </div>
              ))}
            </div>
          </div>
      ...
      <footer className="w-full text-center py-10 border-t border-slate-700 bg-slate-900">
        <p className="text-sm text-slate-500">{t('footer.copyrightPricingPage', { year: new Date().getFullYear(), appTitle: t('appTitle') })}</p>
